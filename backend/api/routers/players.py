...
async def api_upload_avatar(id: str, request: Request):
    ...
    # Validate file extension to ensure it is a safe image type
    ext = file.filename.split(".")[-1].lower() if file.filename else "png"
    if ext not in ["png", "jpg", "jpeg", "webp", "gif"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid image extension. Allowed: png, jpg, jpeg, webp, gif")

    # The `id` is a safe path parameter, ensuring no path traversal vulnerability
