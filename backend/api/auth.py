...
async def verify_hub_password(
    ...
    if not password or not expected_password or not secrets.compare_digest(password.encode('utf-8'), expected_password.encode('utf-8')):
        print("[AUTH] Denied: invalid password attempt")
        raise HTTPException(
