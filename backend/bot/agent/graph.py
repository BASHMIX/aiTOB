...
# Bind the extraction schema to the LLM to force structured output
extractor_llm = llm.with_structured_output(MatchResultExtraction)

def evaluate_chat(state: MatchState) -> dict:
    """
    ...
    try:
        result: MatchResultExtraction = extractor_llm.invoke(prompt)
